...
/// Helper for connecting named fields when overriding 'ConnectField'
#define NS_CONNECT_FIELD(dst_, str_) \
    if (strcmp(str_, name) == 0) \
    { \
        dst_ = Noesis::DynamicCast<decltype(dst_)>(object); \
        NS_ASSERT(dst_ != 0); \
        return true; \
    }

/// Helper for connecting events to code behind functions when overriding 'ConnectEvent'
...
/// Helper for connecting attached events to code behind functions when overriding 'ConnectEvent'
...
public:
    ...
    virtual bool ConnectField(BaseComponent* object, const char* name);
    ...
    /// From UIElement
    //@{
    bool MoveFocus(const TraversalRequest& request) override final;
    DependencyObject* PredictFocus(FocusNavigationDirection direction) override final;
    //@}
...
protected:
    ...
    /// Indicates if element should apply a mirror transform to deal with FlowDirection changes
    bool ShouldApplyMirrorTransform() const;
    ...
    /// From UIElement
    //@{
    Size MeasureCore(const Size& availableSize) override final;
    void ArrangeCore(const Rect& finalRect) override final;
