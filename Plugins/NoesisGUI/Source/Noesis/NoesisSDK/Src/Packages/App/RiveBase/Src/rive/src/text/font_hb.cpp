...
{
    auto blob = noesis_hb_blob_create_or_fail((const char*)span.data(),
                                       (unsigned)span.size(),
    ...
    if (blob)
    {
        auto face = noesis_hb_face_create(blob, 0);
        noesis_hb_blob_destroy(blob);
        if (face)
        {
            auto font = (hb_font_t*)noesis_hb_font_create(face);
            noesis_hb_face_destroy(face);
            if (font)
            {
...
{
    // premable on font...
    noesis_hb_ot_font_set_funcs(font);
    noesis_hb_font_set_scale(font, kStdScale, kStdScale);

    hb_font_extents_t extents;
    noesis_hb_font_get_h_extents(font, &extents);
...
{
    m_drawFuncs = (hb_draw_funcs_t*)noesis_hb_draw_funcs_create();
    noesis_hb_draw_funcs_set_move_to_func(m_drawFuncs, (void*)rpath_move_to, nullptr, nullptr);
    noesis_hb_draw_funcs_set_line_to_func(m_drawFuncs, (void*)rpath_line_to, nullptr, nullptr);
    noesis_hb_draw_funcs_set_quadratic_to_func(m_drawFuncs, (void*)rpath_quad_to, nullptr, nullptr);
    noesis_hb_draw_funcs_set_cubic_to_func(m_drawFuncs, (void*)rpath_cubic_to, nullptr, nullptr);
    noesis_hb_draw_funcs_set_close_path_func(m_drawFuncs, (void*)rpath_close, nullptr, nullptr);
    noesis_hb_draw_funcs_make_immutable(m_drawFuncs);
}

HBFont::~HBFont()
{
    noesis_hb_draw_funcs_destroy(m_drawFuncs);
    noesis_hb_font_destroy(m_font);
}
...
{
    auto featureCount = noesis_hb_ot_layout_language_get_feature_tags(face,
                                                               tag,
                                                               scriptIndex,
                                                               languageIndex,
                                                               0,
    ...
    noesis_hb_ot_layout_language_get_feature_tags(face,
                                           tag,
                                           scriptIndex,
                                           languageIndex,
                                           0,
...
{
    auto scriptCount = noesis_hb_ot_layout_table_get_script_tags(face, tag, 0, nullptr, nullptr);
    auto scripts = std::vector<hb_tag_t>(scriptCount);
    noesis_hb_ot_layout_table_get_script_tags(face, tag, 0, &scriptCount, scripts.data());
    for (uint32_t i = 0; i < scriptCount; ++i)
    {
        auto languageCount =
            noesis_hb_ot_layout_script_get_language_tags(face, tag, i, 0, nullptr, nullptr);

        if (languageCount > 0)
        {
            auto languages = std::vector<hb_tag_t>(languageCount);
            noesis_hb_ot_layout_script_get_language_tags(face,
                                                  tag,
                                                  i,
                                                  0,
                                                  &languageCount,
...
{
    std::unordered_set<uint32_t> features;
    auto face = (hb_face_t*)noesis_hb_font_get_face(m_font);
...
{
    auto face = noesis_hb_font_get_face(m_font);
    assert(index < noesis_hb_ot_var_get_axis_count(face));
    unsigned n = 1;
    hb_ot_var_axis_info_t info;
    noesis_hb_ot_var_get_axis_infos(face, index, &n, &info);
    assert(n == 1);
...
{
    auto face = noesis_hb_font_get_face(m_font);
    return (uint16_t)noesis_hb_ot_var_get_axis_count(face);
}
...
float HBFont::getAxisValue(uint32_t axisTag) const
{
    ...
    uint32_t axisCount = noesis_hb_ot_var_get_axis_count(face);
    for (uint32_t i = 0; i < axisCount; ++i)
    {
        hb_ot_var_axis_info_t info;
        uint32_t n = 1;
        noesis_hb_ot_var_get_axis_infos(face, i, &n, &info);
        if (info.tag == axisTag)
        {
                                          ...
                                          rive::Span<const Feature> features) const
{
    ...
    auto font = (hb_font_t*)noesis_hb_font_create_sub_font(m_font);
    noesis_hb_font_set_variations(font, vars.data(), (unsigned int)vars.size());
...
{
    rive::RawPath rpath;
    noesis_hb_font_get_glyph_shape(m_font, glyph, m_drawFuncs, &rpath);
    return rpath;
}
...
static rive::GlyphRun shape_run(const rive::Unichar text[],
                                ...
                                unsigned textOffset)
{
    hb_buffer_t* buf = (hb_buffer_t*)noesis_hb_buffer_create();
    noesis_hb_buffer_add_utf32(buf, text, tr.unicharCount, 0, tr.unicharCount);

    noesis_hb_buffer_set_direction(buf,
                            tr.dir == rive::TextDirection::rtl ? HB_DIRECTION_RTL
                                                               : HB_DIRECTION_LTR);
    noesis_hb_buffer_set_script(buf, (hb_script_t)tr.script);
    noesis_hb_buffer_set_language(buf, noesis_hb_language_get_default());

    auto hbfont = (HBFont*)tr.font.get();

    noesis_hb_shape(hbfont->m_font,
             buf,
             hbfont->m_features.data(),
             (unsigned int)hbfont->m_features.size());

    unsigned int glyph_count;
    hb_glyph_info_t* glyph_info = (hb_glyph_info_t*)noesis_hb_buffer_get_glyph_infos(buf, &glyph_count);
    hb_glyph_position_t* glyph_pos = (hb_glyph_position_t*)noesis_hb_buffer_get_glyph_positions(buf, &glyph_count);
                                                       ...
                                                       rive::Span<const rive::TextRun> truns) const
{
    ...
    hb_unicode_funcs_t* ufuncs = (hb_unicode_funcs_t*)noesis_hb_unicode_funcs_get_default();
    ...
    while (paragraphStart < text.size())
    {
        ...
        while (runIndex < truns.size())
        {
            ...
            hb_script_t lastScript = (hb_script_t)noesis_hb_unicode_script(ufuncs, text[textIndex]);
            ...
            {
                hb_script_t script = (hb_script_t)noesis_hb_unicode_script(ufuncs, text[textIndex]);
                switch (script)
                {
...
bool HBFont::hasGlyph(rive::Span<const rive::Unichar> missing) const
{
    ...
    return !missing.empty() && noesis_hb_font_get_nominal_glyph(m_font, missing[0], &glyph);
}
