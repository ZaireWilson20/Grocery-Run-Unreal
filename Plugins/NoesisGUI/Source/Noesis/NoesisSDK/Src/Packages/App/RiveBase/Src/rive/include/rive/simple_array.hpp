...
template <typename T, bool IsPOD = std::is_pod<T>::value> class SimpleArrayHelper
{
public:
    static_assert(!std::is_pod<T>::value, "This helper is for non-POD types.");
...
{
public:
    static_assert(std::is_pod<T>::value, "This helper is only for POD types.");
    static void DefaultConstructArray(T* ptr, T* end) {}
    static void CopyConstructArray(const T* first, const T* end, T* ptr)
    {
